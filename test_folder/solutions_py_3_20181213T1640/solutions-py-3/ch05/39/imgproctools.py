##
# Add a function to image processing toolkit that doubles 
# an image in size, replicating each pixel horizonally and 
# vertically. Use processimg.py program to test your function. 

from graphics import GraphicsImage
...
## Doubles an image in size, replicating each pixel horizontally and vertically.
#
def double(image):
    width = image.width()
    height = image.height()
    ...
    for row in range(height):
        for col in range(width):
            ...
            newImage.setPixel(row*2,   col*2,   color)
            newImage.setPixel(row*2+1, col*2,   color)
            newImage.setPixel(row*2,   col*2+1, color)
            newImage.setPixel(row*2+1, col*2+1, color)
