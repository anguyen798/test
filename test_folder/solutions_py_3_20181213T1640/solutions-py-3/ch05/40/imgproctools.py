...
## Places two copies of an image next two each other.
#
def twoCopiesHorizontal(image):
    width = image.width()
    height = image.height()
    newwidth = width*2
    newheight = height
    ...
    for row in range(height):
        for col in range(width):
            ...
            newImage.setPixel(row,col+width,color)

    return newImage

## Places two copies of an image on above the other.
#
def twoCopiesVertical(image):
    width = image.width()
    height = image.height()
    newwidth = width
    newheight = height*2
    ...
    for row in range(height):
        for col in range(width):
            color = image.getPixel(row,col)
            newImage.setPixel(row+height,col,color)
            newImage.setPixel(row,col,color)
