##
# Add a function to image processing toolkit that reduces 
# an image by half, discarding every second pixel. 
...
## Reduces an image by half, discarding every second pixel.
#
def half(image):
    width = image.width()
    height = image.height()
    ...
    for row in range(newheight):
        for col in range(newwidth):
            color = image.getPixel(row*2,col*2)
