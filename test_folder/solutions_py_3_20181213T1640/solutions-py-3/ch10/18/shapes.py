...
## A line segment with an arrow head on one end.
#
class Arrow(Line) :
   ## Overrides the superclass method to draw the line on the canvas.     
   def draw(self, canvas) :
      super().draw(canvas)      
      dx = self._startX - self._endX
      dy = self._startY - self._endY

      angle = atan2(dy, dx)

      y1 = 8 * sin(angle + pi / 6)
      x1 = 8 * cos(angle + pi / 6)
      canvas.drawLine(self._endX, self._endY, self._endX + x1, self._endY + y1)
