...
class GeometricShape :
   ...
   ## Scale the shape by the provided factor, changing its width, height and
   #  location relative to the origin.
   #  @param factor the scale factor (0.5 for half as large)
   #
...
class Rectangle(GeometricShape) :
   ...
   # Overrides the superclass method to scale the rectangle.
   def scale(self, factor) :
      super().scale(factor)
...
class Line(GeometricShape) :
   ...
   # Overrides the superclass method to scale the line.
   def scale(self, factor) :
      super().scale(factor)
      self._startX = self._startX * factor
      self._startY = self._startY * factor
      self._endX = self._endX * factor
      self._endY = self._endY * factor
...
class Group(GeometricShape) : 
   ...
   # Overrides the superclass method to scale the group.
   def scale(self, factor) :
      super().scale(factor)
