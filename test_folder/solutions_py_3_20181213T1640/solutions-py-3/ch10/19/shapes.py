...
class DashedLine(Line) :
   ## Construct a dashed line connecting the provided points using the dash 
   #  and gap lengths provided.
   #  @param x1 the x-coordinate of the starting point
   #  @param y1 the y-coordinate of the starting point
   #  @param x2 the x-coordinate of the ending point
   #  @param y2 the y-coordinate of the ending point
   #  @param dash the length of the dash
   #  @param gap the length of the gap
   #
   def __init__(self, x1, y1, x2, y2, dash, gap) :
   ...
   def draw(self, canvas) :
      ...
      # Find the change in x and y over the length of teh line.
      dx = self._endX - self._startX
      dy = self._endY - self._startY

      # Compute the angle of the line connecting the end points, and its length.
      angle = atan2(dy, dx)
      line_length = sqrt(dx * dx + dy * dy)

      # Determine how long one copy of the dash and gap is in x and y.
      copyx = cos(angle) * (self._dash + self._gap)
      copyy = sin(angle) * (self._dash + self._gap)

      # Determine how long one copy of the dash is in x and y.
      dashx = cos(angle) * self._dash
      dashy = sin(angle) * self._dash

      # Determine how many copies we need and draw that number of copies.
      num_copies = ceil(line_length / (self._dash + self._gap))
      for i in range(0, num_copies) :
         canvas.drawLine(self._startX + i * copyx, self._startY + i * copyy, \
                         self._startX + i * copyx + dashx, \
                         self._startY + i * copyy + dashy)
