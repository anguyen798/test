...
class SnakeGame(GameBase) :
   def __init__(self) :
     ...
     # Problem 30: Game state can be playing (0) or input for a high score (1).
     self._game_state = 0

     # Problem 30: Set up the necessary fonts.
     pygame.font.init()
     self._small_font = pygame.font.SysFont('arial', 20)
     self._big_font = pygame.font.SysFont('arial', 40)

     # Problem 30: Keep track of the information needed for high scores.
     self._initials = ""
     self._high_scores = []
   ...
   def keyDown(self, key) : 
      # Problem 30: Only allow the snake to move when playing the game.
      if self._game_state == 0 and self.getTicks() >= 480 :
         self._snake.keyDown(key)
      else :
         # Problem 30: Handle key presses when entering initials for a high 
         # score.
   ...
   def update(self) :
      ...
      # Problem 30: If the game is currently being played (rather than entering
      # a high score).
      if self._game_state == 0 :
         ...
         if (pygame.sprite.collide_rect(self._snake, self._frog)) and self._frog_alive : 
            ...
            # Problem 30: Compute the score for the current game.
            self._current_score = self.getTicks() - 480
            self._game_end_tick = self.getTicks()
            self.add(self._won) 
   ...
   def draw(self) :
      super().draw()
      if self._game_state == 0 :
         # Problem 30: Draw the high score list.
         ...
         # Problem 30: Display the interface that lets players enter their 
         # initials.
